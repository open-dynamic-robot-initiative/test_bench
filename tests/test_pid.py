...
import yaml
import os
from package_template.pid import get_config_file_pid, get_default_pid, PythonPID, DefaultConfiguration
...
class PID_TESTCASE(unittest.TestCase):
    ...
    def test_config_file_factory(self):
        pid_ctrl = get_config_file_pid(config_file_path=self.YAML_CONFIG_FILE,verbose=False)
    ...
    def test_exception_on_non_existing_config_file(self):
        with self.assertRaises(Exception):
            get_config_file_pid(config_file_path="non_existing_path",verbose=False)
    ...
    def test_integral(self):
        ...
        force_1 = pid_ctrl.compute(position,velocity,position_target,delta_time)
        force_2 = pid_ctrl.compute(position,velocity,position_target,delta_time)
        self.assertNotEqual(force_1,force_2)
        config.ki = 0
        force_3 = pid_ctrl.compute(position,velocity,position_target,delta_time)
        force_4 = pid_ctrl.compute(position,velocity,position_target,delta_time)
        ...
        delta_time = 0.1
        pid_ctrl = PythonPID(config)
        force = pid_ctrl.compute(position,velocity,position_target,delta_time)
        ...
        delta_time = 0.1
        pid_ctrl = PythonPID(config)
        force = pid_ctrl.compute(position,velocity,target_position,delta_time)
